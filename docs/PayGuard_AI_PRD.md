...
| **What are you paying for?** | Product/service description *(optional but recommended)* | "iPhone 15 Pro 256GB", "Freelance logo design", "Shipping charges", "Online course" |
...
6. Critical check: is "refund" or "cashback" mentioned? No one scans a QR to *receive* money — you only scan to *send*.
...
4. `"{product_description}" price India` — market price lookup *(only if product field provided)*
...
*Only runs if the user provided a product/service description.*
...
|---|---|---|
| Agent 2 | No context given, refund framing detected | "Did they ask you to scan this QR to *receive* money, or to pay? (Important: you only scan a QR to send money — never to receive.)" |
...
*PayGuard AI — PRD v3.1 | Band of Agents Hackathon · June 2026 · lablab.ai*
*This is the source of truth. All engineering decisions flow from here.*
